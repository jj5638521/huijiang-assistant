# CHANGELOG（只记录差异）
- 2026-01-05：工资模块升级 **v2026-01-05R76**
  - 固定日薪接入定价链，个人固定日薪覆盖全组/单防撞日单价并输出命中审计。
  - 同一项目内显示名映射同一 name_key 时 Hard 阻断并输出行号证据。
- 2026-01-05：工资模块升级 **v2026-01-05R75**
  - 固定日薪映射新增“马建=300”，并补齐姓名归一冲突的行号证据输出。
- 2026-01-05：工资模块升级 **v2026-01-05R74**
  - 固定日薪/单价映射改为姓名归一 key 匹配，姓名后缀(Pxxx)可命中。
  - 同一项目内多显示名映射同一 key 时 Hard 阻断并输出差异清单。
- 2026-01-05：工资模块升级 **v2026-01-05R73**
  - 选表规则补齐“交叉命中”差值判定：当存在 2 个 CSV 时按差值分数选出勤/报销表，差值均 > 0 且文件不同才放行；否则 Hard 阻断。
  - 选表审计输出新增出勤/报销命中与差值分数。
- 2026-01-05：工资模块升级 **v2026-01-05R72**
  - 增加确定性选表与可审计输出：清洗表头后打分选表，输出映射与选表结果；无法唯一确定直接阻断。
  - 项目池强约束：多项目时必须口令指定项目=xxx，并输出项目清单；指定项目后按项目过滤计算。
  - 是否施工值归一与异常阻断：出勤/施工/是/1/true→是；待命/未施工/否/0/false/空→否；未知值阻断。
  - 姓名规范化支持 “姓名(P007)” 后缀匹配，并记录审计。
